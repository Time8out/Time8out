...
function computeGrossPay(payStructure: PayStructureEntry | null, totalWorkingHours: number, totalDays: number): { grossPay: number; label: string } {
  if (!payStructure) return { grossPay: 0, label: "No pay structure set" };
  const rate = parseFloat(payStructure.Formula);
  const structure = payStructure.Structure?.toLowerCase();
  if (structure === "hourly") {
    return { grossPay: totalWorkingHours * rate, label: `${totalWorkingHours}h × ${rate}/hr` };
  }
  if (structure === "monthly") {
    return { grossPay: rate, label: `Monthly flat rate` };
  }
  if (structure === "daily") {
    return { grossPay: totalDays * rate, label: `${totalDays} days × ${rate}/day` };
  }
  return { grossPay: rate, label: payStructure.Structure };
}
...
export default function Payslip({ employeeID, dateFrom, dateTo, totalWorkingHours, totalDays, totalDeductionMinutes, monetaryDeduction, onClose }: Props) {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (e || !data) { setError("Could not load employee data."); setLoading(false); return; }

      setEmployeeName(`${data.FirstName} ${data.LastName}`);
      setPayStructure(data.PayStructure?.[0] ?? null);
      setCurrency(data.Currency ?? "$");
      setDeductions(data.Deductions ?? []);
      setBonuses(data.BonusAndIncentives ?? []);
      setLoading(false);
    }
    load();
  }, [employeeID]);
  ...
  // ── Computation ──────────────────────────────────────────
  const { grossPay, label: grossLabel } = computeGrossPay(payStructure, totalWorkingHours, totalDays);
  ...
  const netPay = grossPay - totalDeductions - monetaryDeduction + totalBonuses;
              ...
              <>
                ...
                <div className="ps-section">
                  ...
                  <div className="ps-row">
                    <div>
                      <div className="ps-row-label">Basic Pay
                        {payStructure && (
                      ...
                      <div className="ps-row-formula">{grossLabel}</div>
                    </div>
                    <div className="ps-row-amount">{formatCurrency(grossPay, currency)}</div>
                  </div>
                  ...
                  {monetaryDeduction > 0 && (
                    <div className="ps-row">
                      <div>
                        ...
                        <div className="ps-row-formula">{totalDeductionMinutes} mins late/overbreak/early-out</div>
                      </div>
                      <div className="ps-row-amount deduction">−{formatCurrency(monetaryDeduction, currency)}</div>
                    </div>
                  )}
                  {deductionItems.length === 0 && monetaryDeduction === 0 ? (
