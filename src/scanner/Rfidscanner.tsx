...
function RFIDScanner() {
  ...
  async function handleVerify() {
    ...
    if (error || !data) {
      ...
      console.log('[RFIDScanner] Company verified:', data.CompanyCode);
  ...
  const handleKeyDown = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    // Proximity readers send characters rapidly then terminate with Enter
    if (e.key === 'Enter') {
      if (processingRef.current) return;
      processingRef.current = true;
      setTimeout(() => { processingRef.current = false; }, 50);

      const proximityNumber = bufferRef.current.trim();
      if (proximityNumber) {
        // Clear any existing close timer
        if (closeTimerRef.current) clearTimeout(closeTimerRef.current);

        // Show modal immediately with processing state
        setScannedValue(proximityNumber);
        setStampResult(null);
        setShowModal(true);
        setFlash(true);
        setTimeout(() => setFlash(false), 600);

        // ── Resolve the card's ProximityNumber to an EmployeeID first ──
        // Everything downstream (attendance, stamps, OT) keys off EmployeeID,
        // so the card number is only used for this initial identification step.
        const { data: cardOwner, error: cardError } = await supabase
          .from('users')
          .select('EmployeeID')
          .eq('ProximityNumber', proximityNumber)
          .eq('CompanyCode', companyCode)
          .single();

        if (cardError || !cardOwner) {
          console.error('[RFIDScanner] Card not recognized:', cardError?.message);
          setStampResult({ success: false, message: 'Card not recognized. Please try again.' });
        } else {
          setScannedValue(cardOwner.EmployeeID);
          const result = await handleScan(cardOwner.EmployeeID, companyCode);
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            width: 56, height: 56,
            borderRadius: 14,
            background: authState === 'denied' ? '#fee2e2' : '#f3e8ff',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            transition: 'all 0.2s',
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 14, color: '#6b7280', margin: 0, lineHeight: 1.55 }}>
              Enter your company code to activate the RFID scanner.
            </p>
          </div>
          ...
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 8 }}>
            <input
              ...
              onFocus={e => { if (!authError) e.target.style.borderColor = '#7c3aed'; }}
  ...
  return (
    ...
    >
      {/* Hidden input capturing all proximity reader keystrokes */}
      <input
        ref={inputRef}
      ...
      <div style={{ width: '100%', maxWidth: 480, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {/* Scan target */}
        <div style={{
          background: flash ? '#f3e8ff' : '#ffffff',
          border: `2px solid ${flash ? '#7c3aed' : '#e5e7eb'}`,
          borderRadius: 16,
          padding: '32px 24px',
          display: 'flex',
          flexDirection: 'column',
          ...
          gap: 16,
          transition: 'all 0.15s ease',
          boxShadow: flash ? '0 0 0 4px rgba(124,58,237,0.12)' : '0 2px 8px rgba(0,0,0,0.06)',
        }}>
          {/* RFID / proximity card icon */}
          <div style={{
            width: 120, height: 120, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>
            {[
              { top: 0, left: 0, borderTop: `3px solid ${flash ? '#7c3aed' : '#9ca3af'}`, borderLeft: `3px solid ${flash ? '#7c3aed' : '#9ca3af'}` },
              { top: 0, right: 0, borderTop: `3px solid ${flash ? '#7c3aed' : '#9ca3af'}`, borderRight: `3px solid ${flash ? '#7c3aed' : '#9ca3af'}` },
              { bottom: 0, left: 0, borderBottom: `3px solid ${flash ? '#7c3aed' : '#9ca3af'}`, borderLeft: `3px solid ${flash ? '#7c3aed' : '#9ca3af'}` },
              { bottom: 0, right: 0, borderBottom: `3px solid ${flash ? '#7c3aed' : '#9ca3af'}`, borderRight: `3px solid ${flash ? '#7c3aed' : '#9ca3af'}` },
            ].map((style, i) => (
              <div key={i} style={{ position: 'absolute', width: 20, height: 20, borderRadius: 2, transition: 'all 0.15s', ...style as React.CSSProperties }}/>
            ))}
            <svg width="56" height="56" viewBox="0 0 24 24" fill="none" stroke={flash ? '#7c3aed' : '#9ca3af'} strokeWidth="1.8" style={{ transition: 'color 0.15s' }}>
              <rect x="2" y="5" width="20" height="14" rx="2.5" fill="none"/>
              <circle cx="8" cy="12" r="2.3" fill="none"/>
              <path d="M13 9c1.2 0.9 1.2 4.1 0 5" strokeLinecap="round"/>
              <path d="M16 7.2c2.3 1.7 2.3 7.9 0 9.6" strokeLinecap="round"/>
            </svg>
          </div>

          <p style={{ fontSize: 15, color: '#6b7280', margin: 0, textAlign: 'center', lineHeight: 1.5 }}>
            {flash
              ? <span style={{ color: '#7c3aed', fontWeight: 700 }}>Scanned!</span>
              : 'Ready — tap a proximity card now'
            }
          </p>
        </div>
        ...
        >
          <div
            ...
            style={{
              ...
              flexDirection: 'column',
              gap: 20,
              animation: 'rf-up 0.22s cubic-bezier(0.22,1,0.36,1)',
              position: 'relative',
              overflow: 'hidden',
            }}
          >
            ...
            {/* ScannedProfile content — scannedValue holds the resolved EmployeeID */}
            <ScannedProfile
              ...
              <div style={{ width: '100%', height: 3, background: '#f3f4f6', borderRadius: 99, overflow: 'hidden' }}>
                <div style={{
                  ...
                  background: stampResult && !stampResult.success
                    ...
                    : 'linear-gradient(90deg, #7c3aed, #a78bfa)',
                  borderRadius: 99,
                  animation: 'rf-countdown 4s linear forwards',
                }}/>
              </div>
            )}
          </div>
          ...
          <style>{`
            ...
            @keyframes rf-up   { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }
            @keyframes rf-countdown { from { width: 100%; } to { width: 0%; } }
          `}</style>
        </div>
      )}
    </div>
